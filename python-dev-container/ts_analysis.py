...
def johansen_test(df, endogenous_vars, det_order=1, k_ar_diff=1):
    ...
    if cointegration_rank == 0:
        output.append(f"Нет коинтеграционных соотношений между переменными.")
        is_cointegrated = False
    else:
        output.append(f"Обнаружено {cointegration_rank} коинтеграционных соотношений.")
        is_cointegrated = True
...
def generate_time_series_plot(series, title='', figsize=(10, 6), save_path=None):
    """
    Генерация графика временного ряда с настройкой оси X по годам
    ...
    # Функция для настройки оси X с равномерными годами
    def setup_fixed_years_axis(ax):
        years = list(range(series.index.min().year, series.index.max().year + 2))
        
        x_min, x_max = ax.get_xlim()
        positions = np.linspace(x_min, x_max, len(years))
    ...
    # Применяем настройку оси X
    ax = plt.gca()
    setup_fixed_years_axis(ax)
    ...
    if save_path is None:
        ...
        save_path = os.path.join(temp_dir, f'{title.replace(" ", "_")}.png')
...
def generate_time_series_plots_for_variable(df, column):
    """
    Генерация графиков для исходного ряда и первых разностей
    ...
    --------
    dict
        Словарь с путями к графикам
    """
    # Создаем директорию для временных графиков
    temp_dir = 'temp_charts'
    os.makedirs(temp_dir, exist_ok=True)
    
    # Пути для сохранения графиков
    original_path = os.path.join(temp_dir, f'{column}_original.png')
    diff1_path = os.path.join(temp_dir, f'{column}_diff1.png')
    
    # Генерация графика исходного ряда
    generate_time_series_plot(
        df[column], 
        title=f'Исходный ряд: {column}', 
        save_path=original_path
    )
    
    # Генерация графика первых разностей
    diff1 = df[column].diff().dropna()
    generate_time_series_plot(
        diff1, 
        title=f'Первые разности: {column}', 
        save_path=diff1_path
    )
